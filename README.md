...
**[Download the full tool (zip, includes ffmpeg — no install needed)](https://github.com/zohaibhassan-Mindstrom/variations-viewer/releases/latest)**

Unzip it and double-click `Make Frame Board.bat` (right-click →
**Run as administrator** if double-click does nothing). Nothing else to
download or install — ffmpeg is already bundled inside.

> Note: the green **Code → Download ZIP** button on this page only gives
> you this README — the actual tool (scripts + bundled ffmpeg, ~165MB) is
> too large for GitHub to serve that way, so it's distributed as a
> [Release](https://github.com/zohaibhassan-Mindstrom/variations-viewer/releases/latest) instead. Use the link above.
...
- **The tool that builds the board** (`Make Frame Board.bat`) — only works
  on **Windows**. Download it from the
  [Release link above](#-download), unzip, and double-click — it already
  has everything it needs (including **ffmpeg**) bundled inside, so
  there's nothing to separately download or install, ever. It won't run
  on a Mac or Linux machine at all, but on Windows a teammate can just
  copy this whole folder and double-click, nothing to set up first.

## Quick Start

1. **Double-click `Make Frame Board.bat`.**

   **If nothing happens on double-click, right-click it and choose "Run as administrator" instead.**
...
There's nothing to do. The [Release zip](#-download) already includes
its own copy of **ffmpeg** (the tool that does the actual video-reading
and picture-extraction) bundled inside the `ffmpeg` subfolder — no
installing anything, no internet connection needed, ever. Download it,
unzip it, double-click `Make Frame Board.bat`, done.

<details>
<summary>Technical note, only relevant if you're working from the raw
source code instead of the Release zip</summary>
<br>

Source-only checkouts (e.g. from `git clone`) don't include the bundled
`ffmpeg` folder — it's too large for a normal code repository. In that
case, the very first time you run the tool on a computer that doesn't
have ffmpeg yet, it installs it for you automatically (via `winget`, the
installer already built into Windows) before continuing, and needs an
internet connection for that one step. If that computer somehow doesn't
have `winget` either (very rare — it ships with Windows 10/11 by default),
the tool will tell you clearly and give you the one-line command to run
yourself instead. This doesn't apply if you used the Release zip.
</details>
...
| `ffmpeg/` | The bundled copy of ffmpeg — included if you downloaded the [Release zip](#-download) (see [First-Time Setup](#first-time-setup)). Keep it in the same folder — don't rename or move it out. |
| `tests/` | Automated (Pester) tests for the function library. Only relevant if you're changing the code. |

## Requirements

- Windows 10 or 11.
- **ffmpeg** — already bundled if you used the [Release zip](#-download);
  see [First-Time Setup](#first-time-setup) otherwise.
- **Microsoft Edge** for Playable mode (already built into Windows 10/11).
- Nothing else to install. Usually no admin rights needed either — if
  double-clicking `Make Frame Board.bat` does nothing, right-click it and
